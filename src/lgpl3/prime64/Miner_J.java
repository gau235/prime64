 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/srchSrc.jsp?nameOfClass=Miner_J" >src</a>
 *
 * @see Miner_Y
 */
public abstract class Miner_J extends Miner_D {
	 ...
	 */
	public static long[] mineNRetAry(long n1, long n2) { // both inclusive

		int lenByGuess = GuessFromGauss.guessTotalPNRetInt32(n1, n2), iKeep = 0, tmp32Not0;

		// O.l("lenByGuess=" + O.f(lenByGuess), THIS);

		long[] ret = new long[lenByGuess];
		...
		if (n2 < Ranger.$2POW31_MINUS1) {
			...
			int n1_32 = (int) n1, n2_32 = (int) n2;

			for (; n1_32 <= n2_32; n1_32 += ((b4 = !b4) ? 4 : 2)) {

				// O.l("n1_32=" + O.f(n1_32) + O.L + "b4=" + b4, THIS);
				...
				if (testPrime32FromDiv17(n1_32)) {
					...
					if (iKeep == lenByGuess) {
						...
						System.arraycopy(ret, 0, (ret = new long[lenByGuess]), 0, iKeep);

						if (O.isDev) // maybe do again

							O.l("N1=" + O.f(n1) + " N2=" + O.f(n2) + " n1=" + O.f(n1_32) + " n2=" + O.f(n2_32) + " oldL=" +

									O.f(iKeep) + " newL=" + O.f(lenByGuess) + " difL=" + O.f(lenByGuess - iKeep), THIS);
			...
			for (; n1 <= n2; n1 += ((b4 = !b4) ? 4 : 2)) {
				...
				if (testPrimeFromDiv17(n1)) {
					...
					if (iKeep == lenByGuess) {
						...
						if (O.isDev) // maybe again

							O.l("N1=" + O.f(n1) + " N2=" + O.f(n2) + " n1=" + O.f(n1) + " n2=" + O.f(n2) + " oldL=" +
		...
		if (++tmp32Not0 != lenByGuess) // 縮短陣列
			...
			System.arraycopy(ret, 0, (ret = new long[tmp32Not0]), 0, tmp32Not0);
