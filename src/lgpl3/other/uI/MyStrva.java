 ...
 *
 * @version 2022/05/07_00:00:00<br/>
 *          <a target="_blank" href="http://c64.tw/w20/o/srchSrc.jsp?nameOfClass=MyStrva" >MyStrva.java</a>
 *
 */
public abstract class MyStrva {
	...
	public static int countNumOfStringPerLine(long maxPrime) {
		...
		StringBuilder singleStr = new StringBuilder(O.defLenForStr).append(maxPrime).append(O.C44).append(O.C32);
	...
	public static StringBuilder genHeadStr() {
		...
		return new StringBuilder(O.defLenForStr).append(O.C_A_L).append(T64.toTimeStrWGmt()).append(O.C_A_L);
	}
	...
	public static StringBuilder genSummaryOfSeg(StringBuilder strN1, StringBuilder strN2, StringBuilder strTotalP,
		...
		return new StringBuilder(O.defLenForStr).append(O.C_A_L).

				append(strN2).append(" ~ ").append(strN1).append(O.strForIndent).append(strTotalP).append(" p   ").

				append(strTimeCost).append(" s").append(O.C_A_L);

		// O.l("genSummaryOfSeg=" + str, THIS);
	...
	public static StringBuilder genSegFromPrimeAry2D(PrimeAry2D primeAry2D, float timeCost) {
		...
		final int lenMinus = primeAry2D.i - 1;
	...
	public static StringBuilder genAllThrTerminatedStr(StringBuilder strN1, StringBuilder strN2, StringBuilder strTotalP,
		...
		return new StringBuilder(O.defLenForStr).append(strN1).append(" ~ ").append(strN2).

				append(O.strForIndent).append(strTotalP).append(" p   ").append(strTimeCost).append(" s").append(O.C_A_L);
