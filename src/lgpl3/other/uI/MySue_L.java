 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/srchSrc.jsp?nameOfClass=MySue_L" >MySue_L.java</a>
 *
 * @see MySue
 */
public abstract class MySue_L extends MySue_G {
	 ...
	 */
	public static StringBuilder[] toAryOfStrForRowInTable(Recva recva) {

		StringBuilder[] ret = new StringBuilder[MyTablePane.N32_OF_COL];

		int idx = 0;

		ret[idx++] = new StringBuilder(O.f(recva.n1));
		ret[idx++] = new StringBuilder(O.f(recva.n2));

		final CharSequence note = recva.note;

		ret[idx++] = ((note instanceof StringBuilder) ? (StringBuilder) note : new StringBuilder(note));

		ret[idx++] = new StringBuilder(O.f(recva.totalP));
		ret[idx++] = new StringBuilder(String.format(SByN.REGEX_FLOAT, recva.f32TimeCost));

		ret[idx++] = new StringBuilder(T64.timeStr().substring(8, 19));
	 ...
	 */
	public static void insertRowIntoTable(StringBuilder[] aryOfStr) {

		// O.l("aryOfStr=" + aryOfStr, THIS);
		...
		try {
			...
			MyTablePane.TABLE_MODEL.insertRow(0, aryOfStr);

		} catch (Throwable throwable) {

			throwable.printStackTrace();

			new ThrToSendHttpErr(Jsp.toStrByThrowable(throwable, Jsp.L), O.SYS_USER_DOT_NAME);
	...
	public static void writeDatToFile() { // synchronized (allRow)
		...
		TableColumn[] aryOfTablCol = Collections.list(MyTablePane.TABLE_COL_MODEL.getColumns()).

				toArray(MyTablePane.TABLE_COL_ARY0);
		...
		try {
			IOr.toUtf8FileAftNew(defPathOfDirToSaveMP.resolve(MY_FULL_FILE_NAME_TO_REC), str);
