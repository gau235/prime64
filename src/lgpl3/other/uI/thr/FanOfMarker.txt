 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/searchDocOfSrc.jsp?nameOfClass=FanOfMarker" >FanOfMarker.java</a>
 *
 * @see FanOfThrToMine
 *
 * @see FanOfMarker
...
public final class FanOfMarker extends ThrWBox<Marker> {
	...
	public void run() {
		...
		do {
			...
			str.append(MyStrva.STR_4_BLANK).append(String.format(SByN.REGEX_FLOAT, T64.difInF32Sec(tStart))).append(" s");
			str.append(O.C_A_L);
		...
		strDone.append(MyStrva.STR_4_BLANK).append(O.f(recva.totalP)).append(" p").append(MyStrva.STR_4_BLANK);
		strDone.append(String.format(SByN.REGEX_FLOAT, recva.f32TimeCost)).append(" s").append(O.C_A_L);

		MySue.insertRowIntoTable(MySue.genArrAsRowInTable(recva));
		...
		if (O.IS_HIGH_XMX && (oriN1 == 1L) && (oriN2 <= Miner.$MAX_TAIL_N_FOR_GROW_ORI_ARY_OF_P)

				&& Miner.isAnyPFromGtMaxPInOriAryPToN(oriN2)) {

			Miner.setOriAryOfP(box.retrieveP(oriN1, oriN2), THIS);
		}
