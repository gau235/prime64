 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/srchSrc.jsp?nameOfClass=FanOfThrToMine" >FanOfThrToMine.java</a>
 *
 * @see FanOfThrToMine
 *
 * @see FanOfMarker
...
public class FanOfThrToMine extends ThrWBox<Moon<ThrToMine>> {
	...
	public void run() {
		...
		recva.note = new StringBuilder(O.defLenForStr).append("MP").append(SByN.pad0D2(box.numOfThr));

		recva.tmpObj = O.SYS_USER_DOT_NAME;

		/////////////////////////////////////////////////

		CharSequence[] aryOfCharSeq = MySue.toAryOfStrForRowInTable(recva);

		MySue.insertRowIntoTable(MySue.toAryOfStrForRowInTable(recva));

		myTxt.insert(MyStrva.genAllThrTerminatedStr((StringBuilder) aryOfCharSeq[0], (StringBuilder) aryOfCharSeq[1],

				(StringBuilder) aryOfCharSeq[3], (StringBuilder) aryOfCharSeq[4]).toString(), 0);

		if (O.isDev)

			O.l("PrimeAry2D len=" + O.f(pAry2D.i), THIS);
