...
public abstract class CatalanNum {
	...
	public static void winOrLose(final int totalW, long prefix, int countW, int countL, KAryV32 allRet) { // first time must be 0b1L
		...
		if (countW == totalW) {
			...
			if (countW > countL) {
				...
				B32va.lv((countW + countL), "prefix=" + B64W6.strByVCellAftRev(prefix) + " W > L 補滿");
		...
		if (countW > countL) {
			...
			B32va.lv((countW + countL), "prefix=" + B64W6.strByVCellAftRev(prefix) + " W > L");
	...
	public static StringBuilder strMatrixChainMul(long b64W6) { // 原創
		...
		do {
			...
			ret.append(((vCell = (((int) b64W6) & B64W6.MASK32)) == ID_AS_WIN) ? ((char) (++count + vCell)) : O.C42);
