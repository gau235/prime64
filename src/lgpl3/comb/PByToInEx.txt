 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/searchDocOfSrc.jsp?nameOfClass=PByToInEx" >PByToInEx.java</a>
 *
 * @see PInEx
 */
	...
	public static StringBuilder script(int n, int headKToEx, CharSequence lineWr) {
		...
		for (int i = 0; i <= headKToEx; i++) {
			...
			if ((i & 0b1) == 0b0) // 0, 2, 4,...
				...
				// str.a(" +C(" + headKToEx + "," + i + ")*" + (n - i) + "!");
				retStr.append("C(").append(headKToEx).append(O.C44).append(i).append(")*").append(n - i).append("! ");

			else {

				retStr.append("-C(").append(headKToEx).append(O.C44).append(i).append(")*").append(n - i).append(O.C33);
