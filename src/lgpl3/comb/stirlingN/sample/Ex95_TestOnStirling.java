 ...
 *
 * @version 2022/02/11_13:20:00<br/>
 *          <a target="_blank" href="http://c64.tw/w20/o/searchDocOfSrc.jsp?nameOfClass=Ex95_TestOnStirling" >Ex95_TestOnStirling.java</a>
 *
 */
		...
		for (int n = 10, k, min; n != 0; n--)
			...
			for (k = n; (k != 0) && (n * k <= 64); k--)
				...
				for (min = n / k; min >= 0; min--) {
					...
					if (new ThrS2PWDynaB64(condi, B.I).box.iLen != S2P.int64ByDih(condi))

						O.x(condi.toStr());

					if (new ThrS2PCircWDynaB64(condi, B.I).box.iLen != S2PCirc.int64ByDih(condi))
					...
					if ((min != 0) && new ThrOntoPWDynaB64(condi, B.I).box.iLen != OntoP.int64ByHxy(condi))
