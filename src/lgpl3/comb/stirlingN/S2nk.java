 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/searchDocOfSrc.jsp?nameOfClass=S2nk" >S2nk.java</a>
 *
 * @see S2PCirc
 *
 * @see S2nk
		...
		for (int n = 10, k, min; n != 0; n--)
			...
			for (k = n; k != 0; k--)
				...
				for (min = n / k; min >= 1; min--) {
					...
					if (colRecur(condi).iLen != int64ByDih(condi)) // O.eq(colRecur(condi).iLen, int64ByToDiI(condi));

						O.x("n, k, min, max=" + condi.n + ", " + condi.k + ", " + condi.min + ", " + condi.max);

					if (colRecur(condi).iLen != bigIntByDih(condi).longValue())
		...
		for (int n = 10, k, min; n != 0; n--)
			...
			for (k = n; k != 0; k--)
				...
				for (min = n / k; min >= 1; min--) {
					...
					if (colToK(condi).iLen != int64ByDih(condi))
