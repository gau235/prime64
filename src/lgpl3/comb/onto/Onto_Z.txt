 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/searchDocOfSrc.jsp?nameOfClass=Onto_Z" >Onto_Z.java</a>
 *
 * @see Onto
 */
	...
	public static StringBuilder scriptByToInEx(Condi condi, CharSequence lineWr) {
		...
		for (int i = 0, kMinus1 = condi.k - 1; i != condi.k; i++) {
			...
			if ((i & 0b1) == 0b0)
				...
				retStr.append("C(").append(condi.k).append(O.C44).append(i).append(")*").append(condi.k - i).append(O.C94).append(condi.n)
						.append(O.C32);
	...
	public static StringBuilder script(Condi condi, CharSequence lineWr) {
		...
		for (int iAry = 0; iAry != ary.length;) {
			...
			while (b64W6ForC != 0b0L) {
				...
				retStr.append("C(").append(restN).append(O.C44).append(tmp).append(O.C41);
