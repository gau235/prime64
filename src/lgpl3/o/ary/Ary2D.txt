 ...
 *          <a target="_blank" href="http://c64.tw/w20/o/searchDocOfSrc.jsp?nameOfClass=Ary2D" >Ary2D.java</a>
 *
 * @see Ary32_2D
 *
 * @see PrimeAry2D
...
public class Ary2D extends Arr<long[]> {
	...
	public Ary2D sort() {
		...
		Arrays.sort(arr, 0, iLen, CompaForAryOfPrime.COMPA);
	...
	public long[] mergeIn1Ary() { // same as PrimeAry2D_A
		...
		for (; idx != iLen; idx++, totalLen += tmpAry.length)
	...
	public StringBuilder toStr() {
		...
		for (iBig = 0; iBig != iLen;) {

			ary = arr[iBig++];

			for (iSmall = 0; iSmall != ary.length;) {

				retStr.append(ary[iSmall++]);
